...
/* ─── Model constants for routing rule templates. ────────────────
 *  Bump these when the upstream providers update their lineup.       */

/** Model ids used by the built-in routing rule templates.
 *  Kept in one place so they don't rot independently. */
...
const TEMPLATES: RuleTemplate[] = [
  {
    ...
    rule: {
      ...
      target: { provider: "gemini", model: TEMPLATE_MODELS.GEMINI_PRO, headers: {} },
  ...
  },
  {
    ...
    rule: {
      ...
      target: { provider: "anthropic", model: TEMPLATE_MODELS.CLAUDE_HAIKU, headers: {} },
  ...
  },
  {
    ...
    rule: {
      ...
      priority: 5,
      prefer_free: true,
      targets: [{ provider: "gemini", model: TEMPLATE_MODELS.GEMINI_FLASH, headers: {} }],
      target: { provider: "openrouter", model: TEMPLATE_MODELS.NEX_FREE, headers: {} },
  ...
  },
  {
    ...
    rule: {
      ...
      target: { provider: "openai", model: TEMPLATE_MODELS.GPT, headers: {} },
  ...
  },
  {
    ...
    rule: {
      ...
      target: { provider: "openai", model: TEMPLATE_MODELS.GPT, headers: {} },
      targets: [
        { provider: "anthropic", model: TEMPLATE_MODELS.CLAUDE_SONNET, headers: {} },
        { provider: "gemini", model: TEMPLATE_MODELS.GEMINI_PRO, headers: {} },
      ],
...
function RuleEditor({
  ...
  const [activeTab, setActiveTab] = useState<"target" | "match" | "advanced">("target");
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <div className="grid cols-2" style={{ gap: 12 }}>
              <Field
                label="Rule name"
                hint="Unique identifier for logs."
              >
                <input
                  className="input mono"
                  value={rule.name}
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <div className="field" style={{ flex: 1, minWidth: 100, margin: 0 }}>
                <label style={{ fontSize: 11, marginBottom: 4, display: "block" }}>Provider</label>
                <ProviderSelect
              ...
              <div className="field" style={{ flex: 1.5, minWidth: 140, margin: 0 }}>
                <label style={{ fontSize: 11, marginBottom: 4, display: "block" }}>Model</label>
            ...
            <div className="hint" style={{ marginTop: 4 }}>
              Routes to: <strong className="mono">{providerLabel(rule.target.provider)}/{rule.target.model || "(empty)"}</strong>
            </div>
          </div>
          ...
          <div>
            ...
            <Field
              label="Internal note"
              hint="Why this rule exists. Helps you remember months later."
            >
              <input
                className="input"
                value={rule.reason}
                placeholder="e.g. Vision inputs to Gemini..."
      ...
      {activeTab === "match" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <div className="section-head" style={{ margin: "0 0 6px" }}>
              <h2>Match when…</h2>
              <span className="sub">All fields optional (blank = always matches).</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <ChipEditor
                label="Tags (any of)"
                hint="Match if request has any of these tags."
              ...
              <ChipEditor
                label="Tags (all of)"
                hint="Match if request has ALL of these tags."
              ...
              <ChipEditor
                ...
                hint="Match if model name contains substring."
                values={rule.match_model_contains}
                onChange={(next) => onPatch({ match_model_contains: next })}
                placeholder="e.g. gpt-4"
              />
              <Field
                label="Capabilities request needs"
                hint="Toggles that must be true for this rule to fire."
              >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            <div className="section-head" style={{ margin: "0 0 6px" }}>
              ...
              <span className="sub">Unhealthy primary routes to these next.</span>
            </div>
            {rule.targets.length === 0 && (
              <div className="hint" style={{ marginBottom: 8 }}>
                No fallbacks (returns error if primary fails).
              </div>
            )}
            {rule.targets.map((fb, fbIdx) => (
              <div key={fbIdx} className="target-row fallback" style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                <span className="fallback-idx" style={{ fontSize: 11, color: "var(--fg-faint)", minWidth: 20 }}>#{fbIdx + 1}</span>
                ...
                <div className="field" style={{ flex: 1.5, minWidth: 120, margin: 0 }}>
                  <input
                    className="input mono"
                    value={fb.model}
            ...
            <button className="btn" onClick={onAddFallback} style={{ marginTop: 8, alignSelf: "flex-start" }}>
