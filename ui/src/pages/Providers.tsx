  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
        ...
        <div className="field" style={{ margin: 0 }}>
          ...
          <div className="row" style={{ gap: 6 }}>
            <input
              className="input mono"
              style={{ flex: 1 }}
              type={revealedKey !== null || showKey ? "text" : "password"}
              value={revealedKey !== null ? revealedKey : draft.api_key_secret_id}
        ...
        <div className="field" style={{ margin: 0 }}>
          <label style={{ fontSize: 11, marginBottom: 4, display: "block" }}>Model allowlist <span style={{ color: "var(--fg-dim)" }}>(comma separated, leave blank for all)</span></label>
          <input
            className="input mono"
            value={draft.model_allowlist.join(", ")}
            onChange={(e) =>
              setDraft({
                ...draft,
...
export function Providers() {
  ...
  const handleSave = async (d: DraftProvider, isNew: boolean) => {
    ...
    await reload();
    setPageToast({ kind: "ok", msg: `Saved "${payload.display_name}"` });
