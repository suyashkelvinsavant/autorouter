...
const TOOLS: ToolRecipe[] = [
  ...
  {
    name: "OpenCode",
    tagline: "Add an `autorouter` provider + model to ~/.config/opencode/config.json and switch models at runtime via AutoRouter.",
    language: "json",
    build: (b) =>
      JSON.stringify(
        {
...
export function Dashboard({
    ...
    (async () => {
      try {
        ...
        const configured = (data.providers ?? []).filter(
          (p: any) => p.base_url && p.base_url.trim() !== ""
        );
        setHasProviders(configured.length > 0);
      } catch {
  ...
  // Show GettingStarted as an inline banner if no providers are
  // configured — but keep the hero strip, status grid, and gateway
  // URL visible so the operator can still inspect the gateway. The
  // banner is dismissable and sits between the hero and the tool grid.
  const showOnboarding = hasProviders === false;
  ...
  return (
    <>
      ...
      {/* Onboarding banner — shown only when no providers are configured */}
      {showOnboarding && <GettingStarted onNavigate={onNavigate} />}
