...
impl Storage {
    ...
    /// Delete specific session rows by id. Returns the number of rows
    /// removed. Missing ids are ignored (not an error).
    pub fn delete_sessions(&self, ids: &[&str]) -> ConfigResult<usize> {
        if ids.is_empty() {
            return Ok(0);
        }
        let conn = self
    ...
    /// Delete every session whose `updated_at` is strictly less than
    /// `cutoff_unix_secs`. Used by the background session pruner so
    /// the SQLite table cannot grow without bound even when sessions
    /// never re-enter the in-memory registry after a restart.
    pub fn prune_sessions_updated_before(&self, cutoff_unix_secs: i64) -> ConfigResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ConfigError::Storage(e.to_string()))?;
    ...
    /// active entries. The id tie-breaker makes the result deterministic when
    /// multiple writes occur in the same second.
    ///
    /// This complements age-based pruning: after a restart, recent sessions
    /// may exist only in SQLite and therefore cannot be evicted by the
    /// in-memory registry's cap.
    pub fn prune_sessions_to_limit(&self, max_sessions: usize) -> ConfigResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ConfigError::Storage(e.to_string()))?;
...
fn apply_migration(conn: &Connection, version: u32) -> ConfigResult<()> {
    match version {
        ...
        5 => {
            // v5: purge phantom `openai/gpt-5` rows that MockUpstream
            // recorded when the OpenAI slot was unconfigured. This is
            // a data cleanup only — the schema is unchanged.
            let deleted = conn.execute(
                "DELETE FROM provider_events WHERE provider = 'openai' AND model = 'gpt-5'",
                [],
            )?;
            tracing::info!(rows = deleted, "v5: purged phantom MockUpstream rows");
            Ok(())
        }
