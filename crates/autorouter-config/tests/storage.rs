...
#[test]
fn session_pruning_enforces_persistent_cap() {
    let dir = tempdir().unwrap();
    let path = dir.path().join("autorouter.db");
    let s = Storage::open(&path).unwrap();
    for id in ["a", "b", "c"] {
        s.upsert_session(id, None, "openai", None, 0).unwrap();
    }

    assert_eq!(s.prune_sessions_to_limit(2).unwrap(), 1);
    let sessions = s.list_sessions(10).unwrap();
