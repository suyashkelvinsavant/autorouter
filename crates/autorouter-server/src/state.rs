...
/// Build a [`SmartRouter`] from a pipeline, the loaded config, and a
/// [`HealthTracker`]. The smart router is the wiring the headless
/// binary and the desktop shell both use.
    ...
    for adapter in pipeline_adapters(pipeline) {
        for m in adapter.models() {
            reg.register(ModelCapability {
                ...
                // H12: data-driven pricing via a static lookup. Falls
                // back to a name-pattern heuristic for unknown models.
                input_price_per_million: lookup_pricing_input(&m.id),
                output_price_per_million: lookup_pricing_output(&m.id),
                is_free: lookup_pricing_free(&m.id),
            });
        }
    }
    for entry in config.providers.custom.values() {
        for model_id in &entry.model_allowlist {
            reg.register(ModelCapability {
                ...
                input_price_per_million: lookup_pricing_input(model_id),
                output_price_per_million: lookup_pricing_output(model_id),
                is_free: lookup_pricing_free(model_id),
            });
        }
    }
...
/// H12: data-driven pricing table. Returns (input $/M, output $/M,
/// is_free). Unknown models fall back to a name-pattern heuristic
/// (`flash` and `mini` are treated as free) so the router still has
/// something to work with when a new model is added before the
/// pricing table is updated.
/// H12: data-driven pricing table used to populate the
/// capability registry. Returns USD per million input / output
/// tokens. Unknown models return None (and the free heuristic
/// below decides the `is_free` flag).
fn lookup_pricing_input(model_id: &str) -> Option<f64> {
    for (needle, inp, _, _) in PRICING_TABLE {
...
fn lookup_pricing_output(model_id: &str) -> Option<f64> {
    for (needle, _, outp, _) in PRICING_TABLE {
        if model_id.to_ascii_lowercase().contains(needle) {
            return Some(*outp);
        }
    }
