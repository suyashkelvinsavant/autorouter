...
pub fn api_format_to_kind(fmt: ApiFormat) -> ProviderKind {
    match fmt {
        ...
        ApiFormat::Responses => ProviderKind::Custom,
    }
}
...
pub struct MockUpstream {
    ...
    /// When set, `send` / `send_streaming` simulate an upstream HTTP
    /// error with this status code instead of returning a 200 body.
    /// Used by failover tests to exercise the retry cascade.
    failure_status: parking_lot::Mutex<Option<u16>>,
    /// When true, `send` returns `Err(TranslateError::upstream(...))`
    /// for a set `failure_status` (mirroring the real `HttpUpstream`,
    /// which returns `Err` for every non-2xx) instead of `Ok` with the
    /// status. Used to test the production error path, where the
    /// failover layer must still classify 4xx as non-retryable.
    err_mode: parking_lot::Mutex<bool>,
}
...
impl MockUpstream {
    pub fn new(kind: ProviderKind) -> Self {
        Self {
            ...
            failure_status: parking_lot::Mutex::new(None),
            err_mode: parking_lot::Mutex::new(false),
        }
    }
    ...
    /// Make this upstream simulate an HTTP error with the given
    /// status code on every subsequent call. Pass `None` to reset
    /// to normal 200 behaviour.
    pub fn set_error_status(&self, status: Option<u16>) {
        *self.failure_status.lock() = status;
    }

    /// Make `send` return `Err(TranslateError::upstream(...))` for the
    /// configured `failure_status`, mirroring the real `HttpUpstream`
    /// which returns `Err` for every non-2xx. Pass `false` to revert
    /// to the default `Ok`-with-status behaviour.
    pub fn set_error_as_translate_err(&self, on: bool) {
        *self.err_mode.lock() = on;
    }
...
impl UpstreamClient for MockUpstream {
    ...
    async fn send(&self, body: &Value) -> TranslateResult<UpstreamResponse> {
        ...
        // Simulated HTTP error — return a response with the
        // configured status so `call_upstream` / failover logic
        // can exercise the retry path.
        if let Some(err_status) = *self.failure_status.lock() {
            if *self.err_mode.lock() {
                return Err(TranslateError::upstream(format!(
                    "upstream returned {err_status}: mock failure"
                )));
            }
            let err_body = serde_json::json!({
                ...
                "error": { "message": format!("mock upstream error {err_status}") }
            });
            return Ok(UpstreamResponse {
                response: decode_mock_response(&self.kind, &err_body)?,
                status: err_status,
                raw: err_body,
            });
        }
    ...
    async fn send_streaming(&self, body: &Value) -> TranslateResult<UpstreamStream> {
        ...
        if let Some(err_status) = *self.failure_status.lock() {
            return Err(TranslateError::upstream(format!(
                "upstream returned {err_status}: mock streaming failure"
            )));
        }
...
static ERROR_REDACT_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?i)(sk-[A-Za-z0-9]{20,}|Bearer\s+[A-Za-z0-9._\-+/=]{20,}|"[^"]*key[^"]*"\s*:\s*"[^"]+")"#)
        .expect("compile ERROR_REDACT_RE")
});

fn redact_error_body(body: &str) -> String {
    let redacted = ERROR_REDACT_RE.replace_all(body, "[REDACTED]");
...
impl UpstreamClient for HttpUpstream {
    ...
    async fn send(&self, body: &Value) -> TranslateResult<UpstreamResponse> {
        ...
        if !is_success(status) {
            let snippet = redact_error_body(&String::from_utf8_lossy(&bytes));
            return Err(TranslateError::upstream(format!(
    ...
    async fn send_streaming(&self, body: &Value) -> TranslateResult<UpstreamStream> {
        self.check_allowlist(body)?;
        // Full request trace for debugging DeepSeek behavior
        ...
        let first_msg = msgs.and_then(|a| a.first()).map(|m| {
            format!(
                "role={} content_len={}",
        ...
        let tools_names: Vec<String> = body
            .get("tools")
            .and_then(|t| t.as_array())
            .map(|a| {
                a.iter()
            ...
            .unwrap_or_default();
        // Log tool result messages to verify failure propagation
        let tool_msgs: Vec<String> = msgs
            .map(|a| {
                a.iter()
                    .enumerate()
                    ...
                    .map(|(i, m)| {
                        format!(
                            ...
                            m.get("content")
                                .and_then(|c| c.as_str())
                                .map(|s| s.contains("error") || s.contains("EAI_AGAIN"))
                                .unwrap_or(false)
                        )
                    })
                    .collect()
        ...
        if !tool_msgs.is_empty() {
            tracing::info!(tool_results = ?tool_msgs, "⬆ TOOL RESULTS IN REQUEST");
        }
        ...
        tracing::info!(
            ...
            first_msg_role = ?first_msg,
            "⬆ UPSTREAM REQ"
        );

        // Also log the full body at debug level
        tracing::debug!(upstream_body = %serde_json::to_string_pretty(body).unwrap_or_default(), "full upstream request body");
        ...
        if !status.is_success() {
            ...
            let snippet = redact_error_body(&body);
            return Err(TranslateError::upstream(format!(
                "upstream returned {status}: {snippet}"
            )));
        }
        ...
        // Create a per-stream identity token BEFORE the async_stream
        // macro so `stream_request_ptr` is in scope for `StreamGuard::new`.
        // The request is moved into the macro body (captured by move
        // semantics of the generated future).
        ...
        let decoded = async_stream::stream! {
            // `stream_request` was created above; move it into scope so
            // the adapter calls (`decode_bytes`, `decode_text`) can
            // reference it safely across await points.
            ...
            while let Some(chunk_res) = s.next().await {
                match chunk_res {
                    ...
                    Err(e) => {
                        // StreamGuard::drop handles cleanup — just propagate
                        // the error and return so the guard drops cleanly.
            ...
            // StreamGuard::drop calls all cleanup functions when it goes
            // out of scope — no need to call them manually here.
        };
        // Wrap in StreamGuard so cleanup runs even if the client
        // disconnects and the future is dropped before the stream
        // loop reaches its natural end.
        let guard = StreamGuard::new(Box::pin(decoded), stream_request_ptr);
        let stream: UpstreamStream = Box::pin(guard);
        Ok(stream)
    }
}

/// RAII guard that holds the per-stream adapter state key and calls
/// all static-map cleanup functions when it is dropped.
///
/// The global static maps (`STREAMERS`, `TOOL_CALL_IDS`, etc.) inside
/// `autorouter-translate` are keyed by a `stream_id` (`usize`). On
/// the happy path the stream loop ends and the guard drops. When a
/// client disconnects mid-stream, Axum cancels the future at the
/// nearest `await`, so any cleanup after the loop would never run.
/// Moving cleanup into `Drop` covers both cases.
///
/// Cleanup helpers are idempotent (map remove of a missing key is a
/// no-op), so double-drop after a normal end is safe.
///
/// `UpstreamStream` is `BoxStream` (`Send`, not `Sync`). We rely on
/// auto traits — no `unsafe impl Send/Sync`.
struct StreamGuard {
...
impl StreamGuard {
    fn new(inner: UpstreamStream, stream_id: usize) -> Self {
...
impl Drop for StreamGuard {
    fn drop(&mut self) {
        ...
        streamer_drop(id);
        openai_tool_call_drop(id);
        anthropic_tool_call_drop(id);
        gemini_cleanup_drop(id);
        openai_finish_emitted_drop(id);
        anthropic_finish_drop(id);
    }
}

impl futures::Stream for StreamGuard {
    type Item = TranslateResult<autorouter_core::StreamChunk>;

    fn poll_next(
        mut self: std::pin::Pin<&mut Self>,
        cx: &mut std::task::Context<'_>,
    ) -> std::task::Poll<Option<Self::Item>> {
        // `UpstreamStream` is `Pin<Box<dyn Stream + Send>>` and is
        // `Unpin`, so field projection through `Pin` is sound.
