...
/// the same port so the operator can flip CORS without changing bind.
async fn bind_with_reuse(bind: &str) -> std::io::Result<TcpListener> {
    ...
    // `SO_REUSEPORT` is a Unix-only socket option; `socket2` does not even
    // expose `set_reuse_port` on Windows. `SO_REUSEADDR` (set above) is what
    // allows the hot-rebind on Windows, so this call is Unix-only.
    #[cfg(unix)]
