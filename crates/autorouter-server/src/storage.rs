...
impl StorageHandle {
    ...
    /// Delete session rows by id. Missing ids are ignored.
    pub fn delete_sessions(&self, ids: &[&str]) -> Result<usize, String> {
        let guard = self.inner.lock();
        guard.delete_sessions(ids).map_err(|e| e.to_string())
    }

    /// Delete sessions whose `updated_at` is older than the cutoff
    /// (unix seconds). Used by the background session pruner.
    pub fn prune_sessions_updated_before(&self, cutoff_unix_secs: i64) -> Result<usize, String> {
        let guard = self.inner.lock();
        guard
            .prune_sessions_updated_before(cutoff_unix_secs)
            .map_err(|e| e.to_string())
    }

    /// Retain only the most recently active session rows.
    pub fn prune_sessions_to_limit(&self, max_sessions: usize) -> Result<usize, String> {
        let guard = self.inner.lock();
        guard
            .prune_sessions_to_limit(max_sessions)
            .map_err(|e| e.to_string())
    }
