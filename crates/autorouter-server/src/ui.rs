...
pub struct UiState {
    ...
    /// Optional gateway supervisor. The headless binary attaches
    /// the supervisor here (in addition to the `UiAppState.supervisor`
    /// slot used by the HTTP handlers) so the interactive TUI can
    /// hot-rebind the listener when the operator edits
    /// `server.bind`. `None` for desktop tests and unit-test
    /// fixtures. Defaults to `None` via `#[derive(Default)]` on
    /// `Option<GatewaySupervisor>` — adding the field does not
    /// change the existing `Default` impl.
...
///
/// Saves a provider's API key to the secret store and rewrites
/// the entry to a `keychain:` reference. Handles both the legacy
/// single-field (`api_key_secret_id`) and the modern two-field
/// (`api_key_secret_id` + `api_key_value`) form-posting shapes.
...
async fn patch_settings(
    ...
    // Rebuild the smart router when routing inputs change so the
    // next request uses the new defaults/rules/providers.
    ...
    if router_changed {
        ...
        crate::model_db::trigger_scraping_if_needed(&s.app, &router_cfg, s.app.data_dir.as_ref());
    }
...
pub fn mirror_settings_to_storage(storage: &crate::storage::StorageHandle, cfg: &AppConfig) {
    let persist = |key: &str, value: &str| {
        if let Err(e) = storage.set_setting(key, value) {
            tracing::warn!(
                key = key,
                error = %e,
                "failed to persist setting to SQLite storage"
            );
        }
    };
    ...
    persist("bind", &cfg.server.bind);
    persist("enable_cors", &cfg.server.cors_enabled().to_string());
    persist(
        "require_auth",
        &cfg.server.require_auth.unwrap_or(false).to_string(),
    );
    persist("max_body_bytes", &cfg.server.max_body_bytes.to_string());
    persist(
    ...
    persist("default_provider", &cfg.defaults.default_provider);
    persist("default_model", &cfg.defaults.default_model);
    persist(
        "stream_by_default",
    ...
    persist("log_level", &cfg.logging.level);
    persist("log_json", &cfg.logging.json.unwrap_or(false).to_string());
}
...
fn write_config_atomic(path: &std::path::Path, cfg: &AppConfig) -> Result<(), String> {
    ...
    #[cfg(windows)]
    {
        ...
        // MOVEFILE_REPLACE_EXISTING). If that fails (e.g. antivirus
        // holding the file), fall back to copy+delete so the original
        // is never destroyed before we know the new content is safe.
...
async fn patch_routing(
    ...
    // Rebuild the smart router so new rules are live without a restart.
...
/// Build a fresh SmartRouter from the live config and swap it into
/// AppState. Used by PATCH /ui/routing and /ui/settings so the next
/// request is routed by the latest rules + default target.
fn rebuild_and_swap_router(s: &UiAppState) {
    ...
    s.app.replace_router(new_router);
    crate::model_db::trigger_scraping_if_needed(&s.app, &cfg, s.app.data_dir.as_ref());
    LogLine::push(
        &s.ui.log_lines,
        "info",
        "ui",
...
async fn get_events(
    ...
    let events = if provider_filter.is_empty() {
        // Pull events from EVERY provider the gateway has ever
        // served, including custom providers like `openrouter` /
        // `groq`. Previously the kinds list was hardcoded to the
        // three built-in slots and any request routed to a custom
        // provider was invisible on the Requests page — operators
        // saw an empty list (or, worse, a phantom `openai/gpt-5`
        // row when the upstream call had fallen back to the
        // MockUpstream).
        ...
        for name in s.ui.config.read().providers.custom.keys() {
...
async fn get_analytics(
    ...
    authorize(&headers, &s.app)?;
    let storage_guard = s.ui.storage.read();
    // Pull events from every provider the gateway has ever served
    // — built-in slots AND every configured custom provider. The
    // previous hardcoded three-slot list silently dropped events
    // for custom providers, so an OpenRouter-only deployment saw
    // zeros on the Analytics page.
    let mut kinds: Vec<String> = vec![
        "openai".to_string(),
        "anthropic".to_string(),
        "gemini".to_string(),
