...
async fn settings_endpoint_redacts_auth_token() {
    // gap #3: GET /ui/settings must NEVER return the configured
    // bearer credential, even though `apply_settings_patch` stores
    // it verbatim in `cfg.server.auth_token`. The UI replaces it
    // with `null` and adds a sibling `has_auth_token: bool` flag
    // so the page can show "(set — type to replace)".
    let (router, _app, ui) = build();
    {
        let mut cfg = ui.config.write();
...
async fn debug_endpoint_redacts_auth_token_and_env() {
    // gap #3: GET /ui/debug must also redact `server.auth_token`
    // (it's the same credential) and must scrub the value of any
    // `AUTOROUTER_AUTH_TOKEN` env var it would otherwise echo back.
...
async fn patch_settings_response_redacts_auth_token() {
    // gap #3: PATCH /ui/settings must not echo the configured
    // bearer credential back in the response body, otherwise a
    // successful save re-leaks the token into the dashboard's
...
fn redact_auth_token_in_toml_replaces_value_with_empty() {
    // gap #3: the Import/Export page advertises "Secret values
    // are not exported" but `auth_token` was historically
    // written verbatim to `config.toml`. The export endpoint
    // runs every value through this redaction helper before
    // returning the file body.
    let input = r#"[server]
bind = "127.0.0.1:4073"
...
fn redact_auth_token_in_config_clears_field_and_sets_flag() {
    // gap #3: the shared helper used by the HTTP `get_settings` /
    // `patch_settings` and the Tauri `cmd_settings_get` /
    // `cmd_settings_patch` must replace the bearer credential with
    // null and add a sibling `has_auth_token` boolean.
    use serde_json::json;
    let input = json!({
        "server": {
            "bind": "127.0.0.1:4073",
...
// --- gap #2c: HTTP PATCH /ui/settings must toggle CORS on the next request ---
//
// Companion to the supervisor-level tests
// (`sync_router_state_rebinds_on_cors_toggle`): this test drives
// the full HTTP path so we exercise every layer
// (UI handler → RouterBuildState construction → supervisor → axum
// rebuild → next request sees the new CORS layer). Without
// `sync_router_state` and `RouterBuildState`, the live router
// would keep the previous CORS policy until a process restart.
...
async fn http_patch_settings_toggles_cors_live() {
    ...
    assert!(
        ...
        "CORS layer must be GONE after enable_cors=false (gap #2c regression)"
    );

    supervisor.stop_graceful().await;
}

// --- gap #1: HTTP PATCH /ui/routing must hot-reload the smart router ---
//
// The implementation_gaps entry claims "Verified end-to-end: after
// PATCH /ui/routing, the very next POST /openai/v1/chat/completions
// honours the new rule" but until this test that was an ad-hoc curl
// verification. Without a regression test, a future refactor of
// `rebuild_and_swap_router` (e.g. forgetting to call
// `replace_router`) would silently leave the gateway routing with
// the startup-time router and the dashboard's "Save routing" button
// would look like a no-op again.
//
// This test asserts the router on `AppState` changes after the
// PATCH, and that a subsequent decision uses the new rule's
// `reason` field in the metadata.
...
async fn http_patch_routing_hot_reloads_smart_router() {
    ...
    // the startup-time rules (gap #1 regression).
