...
async fn sync_router_state_rebinds_on_cors_toggle() {
    // CORS toggle is a real rebind event — RouterBuildState tracks
    // enable_cors so flipping it triggers a listener rebuild.
    let port = pick_port();
    let bind = format!("127.0.0.1:{port}");
    let supervisor = GatewaySupervisor::new();
    supervisor
...
async fn sync_router_state_rebinds_on_body_limit_change() {
    // max_body_bytes is baked into the router as RequestBodyLimitLayer.
    // Changing it without a rebind keeps the old limit, so the
    // supervisor treats it as a rebind event.
    let port = pick_port();
    let bind = format!("127.0.0.1:{port}");
    let supervisor = GatewaySupervisor::new();
    supervisor
...
async fn sync_router_state_rebinds_on_timeout_change() {
    // request_timeout_seconds is baked into the router. A no-op
    // supervisor would keep the old timeout after a Settings change.
    let port = pick_port();
    let bind = format!("127.0.0.1:{port}");
    let supervisor = GatewaySupervisor::new();
    supervisor
