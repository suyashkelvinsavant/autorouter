...
impl ProviderAdapter for OpenAiResponsesAdapter {
    ...
    fn encode_stream_chunk(&self, chunk: &StreamChunk) -> TranslateResult<Option<String>> {
        // B2: emit SSE frames per call. The Responses API wire format
        // requires lifecycle events (response.created, output_item.added,
        // content_part.added) before text deltas, and closing events
        // before response.completed. ResponsesSseState tracks this.
        let mut state = crate::streaming::ResponsesSseState::default();
        let mut frames = Vec::new();
        for event in &chunk.events {
            frames.extend(crate::streaming::encode_openai_responses_sse(
                event, &mut state, "chunk",
            ));
        }
        let out: String = frames.join("");
        if out.is_empty() {
            Ok(None)
        } else {
            Ok(Some(out))
