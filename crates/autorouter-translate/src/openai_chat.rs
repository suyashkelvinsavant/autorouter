...
/// Tools that identify a request as originating from the Codex CLI (or a
/// Codex-style harness). These names only appear when AutoRouter is acting as
/// the Codex bridge, so their presence is a safe signal that the
/// Codex-specific request rewriting (harness system prompt, system-prompt
/// stripping, planning-tool stripping) should apply. Generic OpenAI Chat
/// traffic never carries them.
fn is_codex_tool(name: &str) -> bool {
    matches!(
        name,
        "exec_command" | "local_shell_call" | "apply_patch" | "shell"
    )
}

/// Detect whether a request is Codex-originated by inspecting its tool set and
/// any tool calls already present in the conversation.
fn request_uses_codex_tools(request: &UniversalRequest) -> bool {
    if request.tools.iter().any(|t| is_codex_tool(&t.name)) {
        return true;
    }
    request.messages.iter().any(|m| {
        m.content.iter().any(|p| match p {
            ContentPart::ToolCall { name, .. } => is_codex_tool(name),
            ContentPart::ToolUse { name, .. } => is_codex_tool(name),
            ContentPart::ToolCallRaw { name, .. } => is_codex_tool(name),
            _ => false,
        })
    })
}
...
impl ProviderAdapter for OpenAiChatAdapter {
    ...
    fn encode_request(&self, request: &UniversalRequest) -> TranslateResult<Value> {
        // Only Codex-sourced traffic needs the harness prompt / system-prompt
        // stripping / planning-tool stripping below. Generic OpenAI Chat
        // requests (Claude Code, ChatGPT clients, the test suite, …) must be
        // encoded verbatim — prepending a Codex harness prompt to every
        // upstream call would corrupt normal conversations. Detect Codex by
        // the tool set it drives (exec_command / apply_patch / …).
        let is_codex = request_uses_codex_tools(request);

        // For Codex traffic, strip ALL system prompts (Codex's own system
        // prompt tells models to explore/plan/update-goals which causes
        // DeepSeek to loop endlessly) and prepend a minimal harness
        // instruction. For generic traffic, preserve the caller's messages
        // and system prompt verbatim.
        let messages: Vec<Value> = if is_codex {
            let filtered: Vec<Value> = request
                .messages
                .iter()
                .filter(|m| {
                    matches!(
            ...
            let mut msgs = vec![json!({
                "role": "system",
                "content": include_str!("codex_harness_prompt.md")
            })];
        ...
        // Strip Codex's planning/goal/MCP tools, but only for Codex traffic.
        ...
        if has_tools {
            ...
            match request.tool_choice.as_deref() {
                ...
                Some("required") => {
                    // DeepSeek/other providers reject "required" in thinking mode
                    body["tool_choice"] = json!("auto");
                }
                Some(tc) => {
                    let parsed: std::result::Result<Value, _> = serde_json::from_str(tc);
                    if let Ok(v) = parsed {
                        body["tool_choice"] = v;
                    } else {
...
/// Annotate a tool JSON with codex_tool classification metadata.
/// Moon-bridge pattern: tools carry extensions that help the response-side
/// reconstruct the correct output item type (local_shell_call, custom_tool_call, etc.)
fn annotate_tool_kind(tool_json: &mut Value, name: &str) {
    if let Some(obj) = tool_json.as_object_mut() {
        if let Some(func) = obj.get_mut("function").and_then(|v| v.as_object_mut()) {
            let kind = match name {
                "exec_command" | "shell" => "exec",
                "apply_patch" => "apply_patch",
                _ => "function",
            };
            func.insert("codex_tool_kind".into(), json!(kind));
            func.insert("codex_openai_name".into(), json!(name));
        }
    }
}
...
fn encode_tool(tool: &ToolDefinition) -> Value {
    ...
    // Moon-bridge: enforce strict schema validation for tool parameters
    if params.is_null() {
        ...
        // Enforce additionalProperties:false so models follow exact schema
        if !obj.contains_key("additionalProperties") {
            obj.insert("additionalProperties".into(), json!(false));
        }
        // Add JSON Schema identifier for validation
    ...
    annotate_tool_kind(&mut tool_json, &tool.name);
    tool_json
}

// TODO(#audit3): Requires explicit `openai_tool_call_drop` call in every
// stream termination path (both in this adapter and in upstream.rs). A
// forgotten call causes an unbounded leak. See reasoning_extractor.rs
// STREAMERS for the RAII-guard recommendation.
...
// TODO(#audit3): Requires explicit `openai_finish_emitted_drop` call in every
