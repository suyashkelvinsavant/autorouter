...
fn decode_openai_chat(body: &serde_json::Value) -> TranslateResult<UniversalRequest> {
        ...
        for t in arr {
            if let Some(func) = t.get("function") {
                let name = func
                    ...
                    .unwrap_or("")
                    .to_string();
                // Skip built-in tools (code_interpreter, file_search,
                // web_search) that lack a function definition — they
                // have no name and must not be forwarded as
                // ToolDefinition.
                if name.is_empty() {
                    continue;
                }
    ...
    Ok(UniversalRequest {
        ...
        temperature: body
            .get("temperature")
            .and_then(|v| v.as_f64())
            .map(|f| f.clamp(f32::MIN as f64, f32::MAX as f64) as f32),
        top_p: body
            .get("top_p")
            .and_then(|v| v.as_f64())
            .map(|f| f.clamp(f32::MIN as f64, f32::MAX as f64) as f32),
...
fn decode_openai_responses(body: &serde_json::Value) -> TranslateResult<UniversalRequest> {
    ...
    if let Some(instructions) = body.get("instructions") {
        if let Some(text) = instructions.as_str() {
            ...
            for item in arr {
                if let Some(item_text) = item.get("text").and_then(|v| v.as_str()) {
                    if !item_text.is_empty() {
                        text_parts.push(item_text.to_string());
                    }
                }
            }
    ...
    if let Some(input) = body.get("input") {
        if let Some(s) = input.as_str() {
            ...
            for raw in arr {
                // OpenAI Responses API tool results arrive as items of
                // `{"type":"function_call_output","call_id":...,"output":...}`
                // inside `input[]` (NOT as `role:"tool"` messages). Map these
                // to a ToolResult part so the upstream model sees the command
                // output. This was previously DROPPED, which meant the model
                // never received command output and the loop guard could not
                // correlate the result with its originating call.
                ...
                if item_type == "function_call_output" {
                    ...
                    let raw_output = raw
                        ...
                        .unwrap_or("")
                        .to_string();
                    // [RELAY FIX] Codex's sandbox sometimes wraps a *successful*
                    // command's output in a spurious "Operation cancelled"
                    // banner (the command actually ran and produced files, but
                    // the approval layer reports cancellation). A capable model
                    // sees "Operation cancelled" and uselessly retries the same
                    // command forever. If the output contains that banner but
                    // ALSO contains real command output (or the command clearly
                    // produced a result), rewrite it to a neutral completion
                    // note so the model treats the step as DONE and moves on.
                    let output = if raw_output.contains("Operation cancelled") {
                        // There is real output alongside the banner — keep the
                        // real output, drop the misleading banner line(s).
                        let cleaned: String = raw_output
                            .lines()
                            .filter(|l| !l.contains("Operation cancelled"))
                            .collect::<Vec<_>>()
                            .join("\n");
                        if cleaned.trim().is_empty() {
                            // The banner was the ONLY content — the command
                            // actually ran (Codex's sandbox reports spurious
                            // cancellation while the command succeeded). Tell
                            // the model it completed so it stops retrying.
                            "[command completed successfully; the 'Operation cancelled' banner from the sandbox was spurious — the command ran and produced output]".to_string()
                        } else {
                            format!(
                                "{}\n\n[NOTE: the sandbox reported 'Operation cancelled' but the command actually completed and produced the output above.]",
                                cleaned
                            )
                        }
                    } else {
                    ...
                    // [RELAY FIX] Pair the tool result with its originating
                    // assistant tool_call. Codex sends the result as a SEPARATE
                    // `/v1/responses` request, so when we relay to an OpenAI
                    // chat upstream the `tool` message would be orphaned
                    // ("Messages with role 'tool' must be a response to a
                    // preceding message with 'tool_calls'"). Reconstruct the
                    // preceding assistant message from CALL_ID_MAP.
                    if let Some((_sig, _run, args)) = crate::streaming::get_call_args(&call_id) {
                        // CALL_ID_MAP stores the call args but not the tool
                        // name; exec_command is the only shell tool we
                        // synthesize a pairing assistant message for here.
                    ...
                    messages.push(Message {
                        ...
                        content: vec![ContentPart::ToolResult {
                            ...
                            content: autorouter_core::ToolResultPayload::Text { text: output },
                            is_error: false,
                        }],
                        name: None,
                    });
    ...
    let temperature = body
        .get("temperature")
        .and_then(|v| v.as_f64())
        .map(|v| v.clamp(f32::MIN as f64, f32::MAX as f64) as f32);
    let top_p = body
        .get("top_p")
        .and_then(|v| v.as_f64())
        .map(|v| v.clamp(f32::MIN as f64, f32::MAX as f64) as f32);
    ...
    debug!(
        instruction_count = messages.iter().filter(|m| matches!(m.role, MessageRole::System)).count(),
        tool_count = tools.len(),
        tool_choice = ?tool_choice,
        "decode_openai_responses"
    );
    Ok(UniversalRequest {
        model,
        system: None,
...
fn decode_gemini(body: &serde_json::Value) -> TranslateResult<UniversalRequest> {
    ...
    let stop = generation_config
        ...
        .unwrap_or_default();
    // Gemini's native protocol carries the model id in the URL path,
    // but callers (Google AI Studio, OpenRouter, Vertex) routinely
    // also send `model` in the body, and many upstreams require it.
    // Honour an explicit body `model` if present; otherwise keep the
    // empty placeholder and let the gateway fill it from the path.
    let model = body
        .get("model")
        .and_then(|v| v.as_str())
        .unwrap_or("")
