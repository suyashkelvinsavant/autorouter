...
// TODO(#audit3): Requires explicit `anthropic_finish_drop` call in every
...
// TODO(#audit3): Requires explicit `anthropic_tool_call_drop` call in every
// stream termination path. See reasoning_extractor.rs STREAMERS for the
// RAII-guard recommendation.
...
fn encode_assistant_content(message: &Message) -> Value {
    ...
    while i < message.content.len() {
        ...
        match part {
            ...
            ContentPart::ToolUse { id, name, input } => blocks.push(json!({
                "type": "tool_use",
                "id": id,
                "name": name,
                "input": input,
