...
/// servers.
// TODO(#audit3): This static map (and the five similar ones in openai_chat.rs,
// anthropic.rs, and gemini.rs) all rely on explicit per-path cleanup calls
// (`streamer_finish` / `streamer_drop`) to avoid leaking entries. If a new
// stream termination path is added and forgets to call the cleanup function,
// the map will grow without bound. A more robust design would use a
// RAII guard (e.g. `StreamGuard`) that cleans up on Drop, so the compiler
// guarantees cleanup regardless of control flow.
