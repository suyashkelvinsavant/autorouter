...
fn anthropic_system_block_list_extracts_text_segments() {
    // The Anthropic system parameter can be an array of content
    // blocks. Verify the decoder extracts text segments and preserves
    // non-text blocks as Unknown.
...
fn anthropic_round_trip_preserves_cache_control_on_system() {
    // Cache_control on the system prompt must survive parse → encode
    // round-trip. Decode keeps the hint; encode emits array-of-blocks.
    use autorouter_core::{ContentPart, MessageRole};
    use serde_json::json;
    let body = json!({
        "model": "claude-3-5-sonnet",
...
fn anthropic_round_trip_preserves_cache_control_on_user_message() {
    // Cache_control on a user-text block must round-trip through
    // the encoder so the upstream honours the cache pin.
    use autorouter_core::{ContentPart, MessageRole};
    use serde_json::json;
    let body = json!({
        "model": "claude-3-5-sonnet",
