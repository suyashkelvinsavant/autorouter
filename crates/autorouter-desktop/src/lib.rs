...
fn perform_shutdown_backup(handles: &ShutdownHandles) {
    ...
    if let Some(storage) = handles.storage.as_ref() {
        ...
        let backup_path = handles.backup_dir.join(format!("autorouter.db.{stamp}"));
...
fn cmd_settings_get(state: tauri::State<DesktopState>) -> Result<Value, String> {
    ...
    // gap #3: never echo the bearer credential back to the webview.
    // Without this, the JS `cfg.server.auth_token` field would
    // contain the secret and any accidental `console.log(cfg)` or
    // IPC message would leak it.
...
async fn cmd_settings_patch(
    ...
    // gap #3: redact the bearer credential from the response so the
    // webview never sees the literal token. Mirrors the HTTP
    // PATCH /ui/settings redaction.
    let v = serde_json::to_value(&cfg).map_err(|e| e.to_string())?;
    Ok(autorouter_server::ui::redact_auth_token_in_config(v))
}
...
fn set_default_provider_model(
    ...
    // R11: rebuild the smart router so the very next request is
    // routed against the new defaults. Mirrors the path
    // `cmd_settings_patch` takes when a routing-relevant field
    // changes (gap #1).
    let smart_router = autorouter_server::build_smart_router(
        &state.app_state.pipeline,
        &cfg,
...
fn cmd_logs(
    ...
    // M22 (cmd_logs since/limit overlap): compute the start index
    // from `since` first, then clamp it so that the slice length is
    // bounded by `limit`. Previously the `if g.len() > limit` branch
    // unconditionally overwrote `start` with `g.len() - limit`, which
    // discarded every line between the `since`-based start and the
    // truncation point.
    let since_start = if let Some(since) = since {
        // M22 (cmd_logs off-by-one): use `<=` so the next poll
        // returns lines STRICTLY newer than `since`. With the
        // previous `<` the partition kept re-including the last
        // line on every poll, so even with no new events arriving
        // the UI kept appending the same line forever - the
        // "keep repeating continuously" symptom. The headless
        // `get_logs` in `autorouter-server/src/ui.rs` already
        // uses `<=`; we are aligning the desktop Tauri command
        // with that semantic.
...
fn cmd_routing_patch(state: tauri::State<DesktopState>, patch: Value) -> Result<Value, String> {
    ...
    state.app_state.replace_config(cfg.clone());
    // R11: rebuild the smart router so the next request is routed
    // by the new rules without a process restart. Mirrors the HTTP
    // PATCH /ui/routing path (gap #1).
    let smart_router = autorouter_server::build_smart_router(
        &state.app_state.pipeline,
        &cfg,
        (*state.app_state.health).clone(),
...
fn cmd_debug(state: tauri::State<DesktopState>) -> Result<Value, String> {
    ...
    let env: Vec<Value> = std::env::vars()
        ...
        // gap #3: scrub the bearer credential from any
        // AUTOROUTER_AUTH_TOKEN env var the same way the HTTP
        // /ui/debug endpoint does, so the webview never sees
        // the literal token.
        .map(|(k, v)| {
    ...
    // gap #3: same redaction as HTTP /ui/debug.
...
/// Desktop mirror of the HTTP `POST /ui/provider_test` handler.
/// Probes a configured provider with a minimal chat-completion
/// request so the dashboard's "Test connection" button can show
/// the operator whether the configured base URL + secret resolve
/// to a working upstream.
///
/// Runs on `tauri::async_runtime` so the `reqwest::send().await`
/// inside `HttpUpstream::send` is driven by Tauri's tokio runtime.
#[tauri::command]
#[allow(dead_code)]
...
fn cmd_export_config(state: tauri::State<DesktopState>) -> Result<String, String> {
    ...
    // gap #3: same redaction as the HTTP /ui/export endpoint. The
    // Import/Export page advertises "Secret values are not
    // exported"; the Tauri path must honour that contract too.
...
pub fn build_gateway() -> Result<Gateway, GatewayError> {
    ...
    //
    // gap #3: the documented default per manual.md is the OS
    // keychain; the env var is an *override* (empty/unset => keychain,
    // explicit "memory" / "file" => opt-in non-defaults).
    ...
    let smart_router = autorouter_server::build_smart_router(&pipeline, &config, health.clone());
    ...
    // so the call sites can clone it freely.
...
pub fn run() {
    // M22 (gap #6): install the in-process log sink BEFORE
    // `init_logging` so the LogSinkLayer (registered as part of the
    // tracing subscriber) can capture tracing events from the very
    // first `record!()` call. Without this call the global `SINK`
    // `OnceLock` is never initialized and `drain_log_sink()` always
    // returns an empty vec - the bridge task would run forever and
    // nothing would ever appear in the Logs page. The headless binary
    // (`autorouter-app/src/main.rs`) already does this; the desktop
    // shell was missing it, which is why the Logs page felt fake and
    // only ever showed the two hand-pushed lines.
    install_log_sink();
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // M22: start the LogBridge so tracing events from the
            // gateway flow into the dashboard Logs page in real time.
            // Start before DesktopState so the handle is captured.
            //
            // The Tauri setup hook is `Fn` (not `async fn`), so we
            // cannot call `tokio::spawn` directly — that would panic
            // with "there is no reactor running" because the current
            // thread is not in a Tokio runtime. We use
            // `LogBridge::start_on_tauri` with
            // `tauri::async_runtime::spawn` as the spawner closure,
            // which routes the task onto Tauri's global async
            // runtime that the framework has already initialised
            // by the time the hook runs. (See the `block_on` calls
            // above — those work because `block_on` enters the
            // runtime itself; calls outside `block_on` need a
            // runtime handle, and Tauri's `async_runtime::spawn`
            ...
            // M22 (gap #6 follow-up): NO MORE hand-pushed fake log
            // lines here. The previous implementation pushed two
            // static lines ("AutoRouter desktop started" and
            // "Gateway bound to {bind}") so the UI had something to
            // show even though the bridge was effectively dead. Those
            // fakes are what made the Logs page feel fake and gave
            // the dashboard a misleading impression of activity.
            // Real tracing events from this point onwards - the
            // `tracing::info!` above ("AutoRouter gateway listening")
            // and every `tracing::*` call from build_gateway, the
            // supervisor, the HTTP handlers, etc. - flow into
            // `log_lines` via the bridge and appear in the UI with
            // their real module-path target.
