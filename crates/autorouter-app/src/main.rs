...
async fn main() -> Result<()> {
    ...
    let smart = autorouter_server::build_smart_router(&pipeline, &config, health.clone());

    // gap #3: per manual.md the documented default is the OS
    // keychain (Windows Credential Manager / macOS Keychain / Linux
    // Secret Service). Previously the unset case fell through to
    // InMemoryStore, which silently discarded API keys on restart.
    // The env var is now an *override*: empty / unset => keychain;
    // "memory" / "file" => explicit non-default choices that the
    // operator can still opt into.
    ...
    // Build the UI sub-router so the headless binary serves the
    // dashboard surface too.
    ...
    // R10 (gap #4 / #2c): the headless binary now owns the same
    // gateway supervisor as the desktop, so PATCH /ui/settings
    // can hot-rebind the listener and pick up router-affecting
    // toggles (`enable_cors`, body limit, request timeout)
    // without a process restart. The supervisor previously lived
    // only on the desktop binary, which meant every CORS toggle
    // on a headless deployment silently did nothing until the
    // operator restarted the process by hand.
    ...
    if config.storage.backup_on_shutdown.unwrap_or(true) {
        // Copy the live SQLite file to a timestamped backup before
        // the process exits, keeping the last backup_keep copies.
        if let Some(storage) = state.storage.as_ref() {
            ...
            if let Some(dir) = backup_dir.as_ref() {
                ...
                let backup_path = dir.join(format!("autorouter.db.{stamp}"));
                if let Err(e) = storage.shutdown(Some(&backup_path)) {
                    tracing::warn!(error = %e, "storage shutdown failed");
                }
                // L2: rotate the rolling backups (keep last N) so the
                // directory does not grow without bound.
                if let Some(dir) = backup_dir.as_ref() {
                    let _ = autorouter_observability::rotate_backup(
                        &dir.join("autorouter.db"),
