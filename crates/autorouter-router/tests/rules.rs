...
fn gap3_runtime_thresholds_block_rule_in_engine() {
    // gap #3: a rule carrying match_latency_below_ms = 100 must
    // not match when the live latency snapshot for the target
    // model is 500ms. The previous implementation accepted the
    // rule unconditionally because passes_runtime_thresholds
    // was never called. This test fails on the old code.
    let mut engine = RuleEngine::with_default(RouteTarget {
        provider: ProviderKind::Gemini,
        model: "gemini-2.5-pro".into(),
        headers: Default::default(),
...
fn gap3_runtime_thresholds_admit_rule_when_within_budget() {
    // gap #3 (positive case): when the live latency is 50ms and
    // the rule allows anything under 100ms, the rule must match
    // and the engine must pick its target.
    let mut engine = RuleEngine::new();
    let target = RouteTarget {
        provider: ProviderKind::OpenAI,
        model: "gpt-5".into(),
...
fn gap3_cost_threshold_blocks_expensive_model() {
    // gap #3 (cost branch): a rule with match_cost_below_per_million
    // = 2.0 must not match when the target model is registered at
    // /M in the capability registry. The telemetry closure here
    // simulates what SmartRouter::decide does in production.
...
fn gap3_untracked_quota_safely_blocks_rule() {
    // gap #3 (safety direction): the engine currently has no quota
    // telemetry. Returning None must make any rule with
    // match_quota_below_pct fail to match, not silently accept.
    // This is the documented \"safe direction\".
    let mut rule = RoutingRule::default_rule(RouteTarget {
        provider: ProviderKind::OpenAI,
        model: "gpt-5".into(),
        headers: Default::default(),
