...
fn health_fallback_picks_best_capability() {
    ...
    // high latency. gap #5: per-model routing — recording against a
    // sibling model (or the legacy empty-model bucket) must NOT
    // penalise `gpt-5`. Only the targeted bucket has to look bad.
    for _ in 0..30 {
        router
            .health()
...
fn m6_fallback_chain() {
    ...
    // gap #5: per-model health — we must hit the actual model ids
    // the router consults, not just the provider kind.
