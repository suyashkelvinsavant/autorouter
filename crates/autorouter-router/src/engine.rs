    ...
    fn decide(
        ...
        let mut decision = match result {
            ...
            Err(_) => {
                ...
                // Iterate ALL scored candidates (not just the top one) and pick
                // the first healthy entry, so a single unhealthy top-candidate
                // does not cause NoRoute when healthy alternatives exist.
                let needs = crate::model::CapabilityNeeds::from_request(request);
                let candidates = self.capabilities.ranked_matches(&needs, false);
                if let Some(fallback) = candidates.into_iter().find(|cap| {
                    self.health
                        .is_model_healthy(cap.provider, &cap.model, self.min_health)
                }) {
                    return Ok(RouteDecision {
