...
impl CapabilityRegistry {
    ...
    pub fn best_match(
        ...
        self.ranked_matches(needs, prefer_free).into_iter().next()
    }

    /// Return all capability-satisfying candidates sorted by score
    /// descending, then by model name for deterministic tie-breaking.
    /// Unlike [`best_match`], this allows the caller to iterate through
    /// candidates and skip unhealthy ones instead of giving up after
    /// checking only the top result.
    pub fn ranked_matches(
        ...
        let mut scored: Vec<(&ModelCapability, i32)> = self
            ...
            .filter(|(_, s)| *s >= 0)
            .collect();
        scored.sort_by(|(a, sa), (b, sb)| sb.cmp(sa).then_with(|| a.model.cmp(&b.model)));
...
impl RoutingRule {
    ...
    pub fn matches(&self, ctx: &RoutingContext) -> bool {
        ...
        // M21: the legacy `when` matcher is a MATCHER, not a hint.
        // Apply it separately so configs behave as documented.
